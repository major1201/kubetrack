PACKAGES = \
github.com/major1201/kubetrack
RELEASE_PLATFORMS = \
darwin/amd64 \
linux/amd64
DOCKER_IMAGE = major1201/kubetrack
GOPROXY = direct
...
deploy:
	#helm template --namespace $(DEPLOY_NAMESPACE) -n $(PROJECTNAME_LOWER) -f ./chart/values.yaml --set "revision=r$$(date +%Y%m%d%H%M%S),image.tag=$(VERSION)" ./chart/$(PROJECTNAME_LOWER)
	helm upgrade --install --namespace $(DEPLOY_NAMESPACE) $(PROJECTNAME_LOWER) -f ./chart/values.yaml --set "revision=r$$(date +%Y%m%d%H%M%S),image.tag=$(VERSION)" ./chart/$(PROJECTNAME_LOWER)
